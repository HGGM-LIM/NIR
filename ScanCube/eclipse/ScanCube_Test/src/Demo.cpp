...
void grid (int Nx,int Ny,int yin, int xin, int stepx, int stepy);

void main(void)
{
    short           ErrorCode,kk,pedo;
	...
	strcpy(path,"D:/amarcos/Documentos/JRipoll/ScanCube/scancube_jaguirre/gridscancube/Correction Files/Cor_1to1.ctb");
	printf("La cadena a cargar es %s\n",path);



    // Initialize
    ErrorCode = load_cor(path);   // load a correction file
                                            // to the SCANalone
    if(ErrorCode)
    {
	...
	printf("set_delay_mode!!!\n");
	printf("SCANalone is ready!\n");
	goto_xy((short)0,(short)0);
	 printf("Ejecutado el goto_xy\n");
	grid(8,8,-4000,-4000,1000,1000);//Nx,Ny;xin;yin;stepx;stepy
	 return;
...
{
//int Nx;
//int Ny; //step scanhead x e y, se calcula longitudbase/numerofuentesxint Ny = 3; //fuentes  x, fuentes menos en y
//int Ns;
//int yin;
//int xin;
//int stepx;
//int stepy;
int Ns;
int Nya; //ayuda para los for
int j;
int v = 0;//para los bucles
int vi;
int ii;
int *x; //apunta a la posicion de memoria donde empezara el array.
int *y; //apunta a la posicion de memoria donde empieza el array.
int *xa;
int *xab;//xa al reves.
int *xaab;//xa + xab
Ns=Nx*Ny;
...
printf("Reserva memoria\n");
	x=(int *)malloc(sizeof(int)*Ns);//creando memoria para el array del tamño adecuado
    y=(int *)malloc(sizeof(int)*Ns);
    xa=(int *)malloc(sizeof(int)*Nx);
	xab=(int *)malloc(sizeof(int)*Nx);
	xaab=(int *)malloc(sizeof(int)*2*Nx);

	printf("Calcula posiciones\n");
	for (j=0;j<Nx;j++)
	{
	xa[j]=xin+stepx*j;
	}
   ...
   for(j=0;j<Ny;j++)
   {
	   ...
	   for (ii=0;ii<Nx;ii++)
	   {
	   y[ii+(j*Nx)]=yin+(j*stepy);
	   }
   }

   printf("Empezamos a mover\n");
for (j=0;j<Ns;j++){
goto_xy((short)x[j],(short)y[j]);
Sleep(100);
...
//
//    // Initialize
//    printf("Loading Correction file\n");
...
//    printf("Timing, delay and speed preset\n");
...
//    printf("wait until the execution is finished\n");
